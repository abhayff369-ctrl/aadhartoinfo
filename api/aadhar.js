// Hardcoded multi-key system: abhay1-5
const VALID_KEYS = [
  'abhay1',
  'abhay2',
  'abhay3',
  'abhay4',
  'abhay5'
];

export default async function handler(req, res) {
  // Enable CORS
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'GET, OPTIONS');
  res.setHeader('Content-Type', 'text/plain; charset=utf-8');
  
  if (req.method === 'OPTIONS') return res.status(200).end();

  const { exploits, api_key } = req.query;

  // Multi-Key Authentication
  if (!api_key) {
    return res.status(401).send('Error: Missing API key\nUsage: ?api_key=abhay1&exploits=123456789012\nValid keys: abhay1, abhay2, abhay3, abhay4, abhay5');
  }

  if (!VALID_KEYS.includes(api_key)) {
    return res.status(403).send('Error: Invalid API key\nValid keys: abhay1, abhay2, abhay3, abhay4, abhay5');
  }

  // Validate Aadhaar (12 digits)
  if (!exploits) {
    return res.status(400).send('Error: Missing Aadhaar number\nUsage: ?api_key=KEY&exploits=123456789012');
  }

  const aadhaarRegex = /^\d{12}$/;
  if (!aadhaarRegex.test(exploits)) {
    return res.status(400).send('Error: Invalid Aadhaar number. Must be exactly 12 digits.');
  }

  const targetUrl = `https://exploitsindia.site/api/aadhar.php?exploits=${exploits}`;

  try {
    const response = await fetch(targetUrl, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; VercelBot/1.0)',
        'Accept': 'text/html,application/xhtml+xml,application/xml'
      }
    });

    let content = await response.text();

    // Remove BUY/SUPPORT lines and extra separators
    const removeBuySupport = /(💳\s*BUY\s+API\s*:\s*@\S+\s*|🆘\s*SUPPORT\s*:\s*@\S+\s*|━━━━━━━━━━━━━━━━━━━━━━━━━━━)/gi;
    content = content.replace(removeBuySupport, '');

    // Clean up multiple newlines
    content = content.replace(/\n{3,}/g, '\n\n').trim();

    // Ensure developer credit is at the bottom
    const developerCredit = '\n\n────────────────────────\n🔧 developer by abhay singh';
    
    // Remove any existing developer credit lines (to avoid duplication)
    const removeExistingCredit = /━━━━━━━━━━━━━━━━━━━━━━━━━━━\s*\n?\s*🔧\s*developer\s+by\s+abhay\s+singh/gi;
    content = content.replace(removeExistingCredit, '');
    
    // Append fresh credit
    content = content + developerCredit;

    console.log(`[KEY_USED] ${api_key} accessed Aadhaar: ${exploits}`);

    res.status(response.status).send(content);
  } catch (error) {
    console.error('Scraping error:', error);
    res.status(500).send('Error: Failed to fetch from target\nDetails: ' + error.message);
  }
}
